...
import erpnext
from masar_cost.override._bin import update_qty as update_bin_qty
...
def repost_current_voucher(args, allow_negative_stock=False, via_landed_cost_voucher=False):
    if args.get("actual_qty") or args.get("voucher_type") == "Stock Reconciliation":
        ...
        update_qty_in_future_sle_for_all_warehouses(args, allow_negative_stock)
...
def repost_future_sle(args=None, doc=None, voucher_type=None, voucher_no=None, allow_negative_stock=None, via_landed_cost_voucher=False):
    ...
    distinct_item = get_distinct_item(args, doc)
    ...
    while i < len(args):
        ...
        obj = update_entries_after_for_all_warehouses({
            "item_code": args[i].get('item_code'),
            "warehouse": args[i].get('warehouse'),
            "posting_date": args[i].get('posting_date'),
            "posting_time": args[i].get('posting_time'),
            "creation": args[i].get("creation"),
            "distinct_item": distinct_item
        }, allow_negative_stock=allow_negative_stock, via_landed_cost_voucher=via_landed_cost_voucher)

        distinct_item[(args[i].get('item_code'))].reposting_status = True

        if obj.new_items_found:
            for item_wh, data in iteritems(distinct_item):
        ...
        if doc and i % 2 == 0:
            update_args_in_repost_item_valuation(doc, i, args, distinct_item)

    if doc and args:
        update_args_in_repost_item_valuation(doc, i, args, distinct_item)

def update_args_in_repost_item_valuation(doc, index, args, distinct_item):
    frappe.db.set_value(doc.doctype, doc.name, {
        'items_to_be_repost': json.dumps(args, default=str),
        'distinct_item_and_warehouse': json.dumps({str(k): v for k,v in distinct_item.items()}, default=str),
        'current_index': index
    })
...
def get_distinct_item(args=None, doc=None):
    distinct_item = {}
    for i, d in enumerate(args):
        distinct_item.setdefault((d.item_code), frappe._dict({
            "reposting_status": False,
            "sle": d,
            "args_idx": i
        }))
...
#--------------------------------------------------------------------
#Class For All Warehouses
#--------------------------------------------------------------------
class update_entries_after_for_all_warehouses(object):
    """
        update valution rate and qty after transaction
        from the current time-bucket onwards
    ...
    def __init__(self, args, allow_zero_rate=False, allow_negative_stock=None, via_landed_cost_voucher=False, verbose=1):
        ...
        self.new_items_found = False
        self.distinct_item = args.get("distinct_item", frappe._dict())
        self.data_item = frappe._dict()
        self.initialize_previous_data(self.args)
        self.build()
    ...
    def initialize_previous_data(self, args):
        ...
        """
        self.data_item.setdefault(args.item_code, frappe._dict())
        item_dict = self.data_item[args.item_code]
    ...
    def build(self):
        from masar_cost.override._stock_controller import future_sle_exists
    ...
    def get_future_entries_to_fix(self):
        # includes current entry!
        args = self.data_item[self.args.item_code].previous_sle \
            or frappe._dict({"item_code": self.item_code})
    ...
    def get_dependent_entries_to_fix(self, entries_to_fix, sle):
        ...
        if not dependant_sle:
            ...
            self.update_distinct_item(dependant_sle)
            return entries_to_fix
        elif dependant_sle.item_code == self.item_code:
            return entries_to_fix
        else:
    ...
    def update_distinct_item(self, dependant_sle):
        key = (dependant_sle.item_code)
        val = frappe._dict({
            "sle": dependant_sle
        })
        if key not in self.distinct_item:
            self.distinct_item[key] = val
            self.new_items_found = True
        else:
            existing_sle_posting_date = self.distinct_item[key].get("sle", {}).get("posting_date")
            if getdate(dependant_sle.posting_date) < getdate(existing_sle_posting_date):
                val.sle_changed = True
                self.distinct_item[key] = val
                self.new_items_found = True
    ...
    def append_future_sle_for_dependant(self, dependant_sle, entries_to_fix):
        ...
        args = self.data_item[dependant_sle.item_code].previous_sle \
            or frappe._dict({"item_code": self.item_code})
        future_sle_for_dependant = list(self.get_sle_after_datetime(args))
    ...
    def validate_negative_stock(self, sle):
        """
            ...
            will not consider cancelled entries
        """
        diff = self.wh_data.qty_after_transaction_for_all_warehouses + flt(sle.actual_qty)
    ...
    def update_outgoing_rate_on_transaction(self, sle):
        ...
        """
        if sle.actual_qty and sle.voucher_detail_no:
            outgoing_rate = abs(flt(sle.stock_value_difference_for_all_warehouses)) / abs(sle.actual_qty)
    ...
    def get_serialized_values(self, sle):
        ...
        if incoming_rate < 0:
            # wrong incoming rate
            incoming_rate = self.item_data.valuation_rate
        ...
        new_stock_qty = self.item_data.qty_after_transaction_for_all_warehouses + actual_qty

        if new_stock_qty > 0:
            new_stock_value = (self.item_data.qty_after_transaction_for_all_warehouses * self.item_data.valuation_rate) + stock_value_change
            if new_stock_value >= 0:
                # calculate new valuation rate only if stock value is positive
                # else it remains the same as that of previous entry
    ...
    def get_fallback_rate(self, sle) -> float:
        """When exact incoming rate isn't available use any of other "average" rates as fallback.
            This should only get used for negative stock."""
        return get_valuation_rate_for_all_warehouses(sle.item_code, sle.warehouse,
            sle.voucher_type, sle.voucher_no, self.allow_zero_rate,
            currency=erpnext.get_company_currency(sle.company), company=sle.company)

    def get_sle_before_datetime(self, args):
        """get previous stock ledger entry before current time-bucket"""
        sle = get_stock_ledger_entries_for_all_warehouses(args, "<", "desc", "limit 1", for_update=False)
        sle = sle[0] if sle else frappe._dict()
        return sle

    def get_sle_after_datetime(self, args):
        """get Stock Ledger Entries after a particular datetime, for reposting"""
        return get_stock_ledger_entries_for_all_warehouses(args, ">", "asc", for_update=True, check_serial_no=False)
    ...
    def update_bin(self):
        #Updated By KCSC on 12/3/2022
        # update bin for each warehouse
        for item_code, data in self.data_item.items():
            for d in frappe.get_list("Bin", fields=("name"), filters={"item_code": self.item_code}):
                bin = frappe.get_doc('Bin', d.name)
                frappe.db.set_value('Bin', d.name, {
                    "valuation_rate": data.valuation_rate,
...
def get_stock_ledger_entries_for_all_warehouses(previous_sle, operator=None,
    order="desc", limit=None, for_update=False, debug=False, check_serial_no=True):
    """get stock ledger entries filtered by specific posting datetime conditions"""
    conditions = " and timestamp(posting_date, posting_time) {0} timestamp(%(posting_date)s, %(posting_time)s)".format(operator)
...
def get_valuation_rate_for_all_warehouses(item_code, warehouse, voucher_type, voucher_no,
    allow_zero_rate=False, currency=None, company=None, raise_error_if_no_rate=True):
...
def update_qty_in_future_sle_for_all_warehouses(args, allow_negative_stock=False):
    """Recalculate Qty after Transaction in future SLEs based on current SLE."""
    datetime_limit_condition = ""
    qty_shift = args.actual_qty
    ...
    # find the next nearest stock reco so that we only recalculate SLEs till that point
    next_stock_reco_detail = get_next_stock_reco_for_all_warehouses(args)
    if next_stock_reco_detail:
        detail = next_stock_reco_detail[0]
        # add condition to update SLEs before this date & time
        datetime_limit_condition = get_datetime_limit_condition(detail)
    ...
    frappe.db.sql("""
        update `tabStock Ledger Entry`
        set qty_after_transaction_for_all_warehouses = qty_after_transaction_for_all_warehouses + {qty_shift}
        where
            item_code = %(item_code)s
            and voucher_no != %(voucher_no)s
            and is_cancelled = 0
...
def get_next_stock_reco_for_all_warehouses(args):
    """Returns next nearest stock reconciliaton's details."""
