...
from erpnext.stock.stock_ledger import update_entries_after as original_update_entries_after
...
class update_entries_after(object):
    ...
    def initialize_previous_data(self, args):
        ...
        item_dict.update({
            "prev_stock_value": previous_sle_for_all_warehouses.stock_value or 0.0,
            "stock_queue": json.loads(previous_sle_for_all_warehouses.stock_queue or "[]"),
    ...
    def process_sle(self, sle):
        ...
        if (sle.serial_no and not self.via_landed_cost_voucher) or not cint(self.allow_negative_stock):
            ...
            if not self.validate_negative_stock_for_all_warehouse(sle):
                self.item_date.qty_after_transaction += flt(sle.actual_qty)
                return
        ...
        # update current sle
        sle.qty_after_transaction = self.item_date.qty_after_transaction
        ...
        sle.stock_value = self.item_date.stock_value
        sle.stock_value_for_all_warehouses = self.item_date.stock_value
        sle.stock_queue = json.dumps(self.item_date.stock_queue)
    ...
    def validate_negative_stock_for_all_warehouses(self, sle):
        """
            validate negative stock for entries current datetime onwards
            will not consider cancelled entries
        """
        ...
        for item_code, data in self.data_item.items():
            ...
            for d in frappe.get_list("Bin", fields=("name"), filters={"item_code": self.item_code}):
                ...
                frappe.db.set_value('Bin', d.name, {
                    ...
                    "stock_value": bin.actual_qty * flt(data.valuation_rate)
                })
